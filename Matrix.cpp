...
Matrix Matrix::VectorProduct(Matrix m1, Matrix m2){
    Matrix res(3, 1);
    if (m1.data.size() == 3 && m1.data[0].size() == 1 && m2.data.size() == 3 && m2.data[0].size() == 1){
        res(0, 0) = m1(1, 0) * m2(2, 0) - m1(2, 0) * m2(1, 0);
        res(1, 0) = m1(2, 0) * m2(0, 0) - m1(0, 0) * m2(2, 0);
        res(2, 0) = m1(0, 0) * m2(1, 0) - m1(1, 0) * m2(0, 0);
    }
    else{
        throw std::logic_error("Something bag, this not planned!");
    }
